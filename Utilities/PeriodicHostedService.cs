                ...
                await timer.WaitForNextTickAsync(stoppingToken))
            {
                try
                {
                    if (IsEnabled)
                    {
                        ...
                        SendDiscountCodeTask sampleService = asyncScope.ServiceProvider.GetRequiredService<SendDiscountCodeTask>();
                        await sampleService.DoSomethingAsync();

                        // Sample count increment
                        _executionCount++;
                        _logger.LogInformation(
                            $"Executed CloseOpenPaymentAttmpTask - Count: {_executionCount}");
                    }
                    else
                    {
                        _logger.LogInformation(
